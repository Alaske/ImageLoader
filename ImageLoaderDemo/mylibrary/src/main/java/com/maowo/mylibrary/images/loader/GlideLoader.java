package com.maowo.mylibrary.images.loader;
...
import com.maowo.mylibrary.images.config.AnimationMode;
import com.maowo.mylibrary.images.config.GlobalConfig;
import com.maowo.mylibrary.images.config.PriorityMode;
import com.maowo.mylibrary.images.config.ScaleMode;
import com.maowo.mylibrary.images.config.ShapeMode;
import com.maowo.mylibrary.images.config.SingleConfig;
import com.maowo.mylibrary.images.utils.DownLoadImageService;
import com.maowo.mylibrary.images.utils.ImageUtil;
...
public class GlideLoader implements ILoader {
    ...
    public void request(final SingleConfig config) {
        ...
        requestBuilder = requestBuilder.apply(requestOptions);//应用RequestOptions
        ...
        //设置图片加载动画
        requestBuilder = setAnimator(config, requestBuilder);
    ...
    private RequestOptions getRequestOptions(SingleConfig config) {
        ...
        //设置图片加载优先级
        options = setPriority(config, options);

        if (config.getErrorResId() > 0) {
            options = options.error(config.getErrorResId());
        }

        options = setShapeModeAndBlur(config, options);//设置RequestOptions 关于 多重变换
//        options = options.transform(new SketchFilterTransformation());
        return options;
    }
     ...
     * @param options
     */
    private RequestOptions setPriority(SingleConfig config, RequestOptions options) {
     ...
     * @param request
     */
    private RequestBuilder setAnimator(SingleConfig config, RequestBuilder request) {
        if (config.getAnimationType() == AnimationMode.ANIMATIONID) {
            ...
            request = request.transition(genericTransitionOptions);
     ...
     * @param request
     */
    private RequestOptions setShapeModeAndBlur(SingleConfig config, RequestOptions request) {
...
//                count++;
        ...
        if (transformation.length != 0) {
            request = request.transforms(transformation);
        }
        return request;
    }
